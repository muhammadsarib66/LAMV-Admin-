...
export function Employees() {
  ...
  const handleOpen = () => dispatch(setisModalOpen());

  const handleSearch = (e)=>{
    const value = e.target.value;
    if(value === '') setTableData(TABLE_ROWS)
    else{
      setTableData(TABLE_ROWS.filter((item)=>item.name.toLowerCase().includes(value.toLowerCase())))
    }
  ...
  const handleFilter = (value)=>{
    if(value === 'all'){
      ...
      setTableData(TABLE_ROWS.filter((item)=>item.online === true))
    }
    else {
      setTableData(TABLE_ROWS.filter((item)=>item.online !== true))
      console.log(value)
  ...
  return (
    ...
    <Card className="h-full w-full mb-10">
          ...
          <Tabs value="all" className="w-full md:w-max">
            <TabsHeader>
              {TABS.map(({ label, value }) => (
                <Tab onClick={()=>handleFilter(value)} key={value} value={value}>
                  &nbsp;&nbsp;{label}&nbsp;&nbsp;
                </Tab>
              ))}
            </TabsHeader>
      ...
      <CardBody className="overflow-auto  h-[70vh] px-0">
