...
export default function EditCustomerModal({ userId }) {
  ...
  const handleActive = async () => {
    await axios
      .post(`${baseUrl}users/unblock-user`, { userId: userId?._id }, config)
      .then((resp) => {
  ...
  const handleBlock = async () => {
    await axios
      .post(`${baseUrl}users/block-user`, { userId: userId?._id }, config)
      .then(() => {
        ...
        <Box sx={style} className=" flex flex-col gap-4 p-4">
          <div className="flex justify-between  ">
            <h1 className="text-xl font-semibold ">
              ...
              {userId && userId.isActive ? "Active" : "Blocked"}
            </h1>
          ...
          <div className="flex justify-end gap-2">
            {userId?.isActive ? (
              <Button onClick={handleBlock} variant="standard">
                block
              </Button>
            ) : (
              <Button onClick={handleActive} variant="outlined">
                Unblock
              </Button>
            )}
          </div>
